...
function Field({
  ...
  const baseInput = `w-full bg-[#f8f9fb] border rounded-xl text-[#0f172a] text-sm outline-none transition-all duration-200
    placeholder-transparent resize-none
    focus:bg-white focus:ring-2
    ${error ? "border-red-400 focus:border-red-400 focus:ring-red-100" : "border-gray-200 focus:border-blue-500 focus:ring-blue-50"}`;
    ...
    <div className="relative">
      ...
      <label
        className={`absolute left-11 pointer-events-none font-medium transition-all duration-200 z-10
          ${focused || hasVal
            ? "top-2 text-[10px] tracking-wider uppercase text-blue-600"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          >
            <div className="inline-flex items-center gap-2 mb-6">
              ...
              <span className="text-blue-600 text-xs font-bold tracking-[0.3em] uppercase">Send a Message</span>
            </div>
            <h2 className="font-display text-4xl sm:text-5xl font-bold text-[#0f172a] leading-tight mb-5">
              Let's Start a
              <br />
              <span className="italic text-blue-600">Conversation</span>
            </h2>
                ...
                {!submitted ? (
                  <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                    ...
                    <div className="flex justify-end -mt-3">
                      <span className={`text-xs font-medium ${form.message.length > 500 ? "text-red-400" : "text-gray-300"}`}>
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full bg-[#0f172a] hover:bg-[#1e293b] disabled:bg-gray-300 disabled:cursor-not-allowed
                        text-white font-bold py-3.5 rounded-xl transition-all duration-200
                        shadow-lg shadow-gray-900/20 hover:shadow-xl flex items-center justify-center gap-2 text-sm"
                    >
                      {loading
                  ...
                  <div className="py-8 text-center">
                    ...
                    <h3 className="font-display text-3xl font-bold text-[#0f172a] mb-3">Message Sent!</h3>
                    <p className="text-gray-400 text-sm leading-relaxed max-w-xs mx-auto mb-2">
                      Thank you, <span className="font-semibold text-[#0f172a]">{form.name}</span>.
                      We've received your message and will reply to
                    </p>
                    <div className="inline-flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2 mb-7">
                      <Mail className="w-3.5 h-3.5 text-blue-500 flex-shrink-0" />
                      <span className="text-blue-600 font-semibold text-sm break-all">{form.email}</span>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      <button
                        onClick={reset}
                        className="flex items-center justify-center gap-2 bg-[#0f172a] hover:bg-[#1e293b] text-white font-bold px-6 py-3 rounded-xl transition-all text-sm shadow-md"
                      >
                        Send Another Message
                      </button>
                      <a
                        href="/"
                        className="flex items-center justify-center gap-2 border border-gray-200 text-gray-600 hover:bg-gray-50 font-semibold px-6 py-3 rounded-xl transition-all text-sm"
                      >
                        Back to Home
                      </a>
                    </div>
