...
import { ShoppingBag } from "lucide-react";
import { WishlistButton } from "./Wishlistbutton";
import { toggleWishlist, getWishlist } from "@/api/wishlist"; // Import your wishlist APIs
import type { Product } from "@/api/seasonalProducts";
  ...
  const checkWishlistStatus = async () => {
    try {
      ...
      if (response && response.success && response.wishlist) {
        ...
        const isInWishlist = response.wishlist.items?.some(
          (item: any) => item.product?._id === product._id || item.productId === product._id
        );
        setWished(isInWishlist || false);
      }
    } catch (error) {
  ...
  const handleWishlistToggle = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        // Toggle the wishlist state based on response
        setWished(response.isInWishlist || !wished);
        ...
        alert(response?.message || "Failed to update wishlist. Please try again.");
      }
    } catch (err) {
      console.error("Wishlist error:", err);
    ...
    >
      ...
      >
        <WishlistButton
          active={wished}
          onToggle={handleWishlistToggle}
          loading={loading || checkingWishlist}
        />
