...
export default function ProductImageGallery({ images = [] }: Props) {
  ...
  return (
    <div className="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4">
      ...
      {/* Main Image */}
      <div
        className="relative flex-1 rounded-2xl overflow-hidden bg-[#f0f0ee] group"
        style={{ aspectRatio: "3/4", minHeight: "320px" }}
      >
        <img
          src={images?.[active]?.src || "/placeholder.png"}
        ...
        {!zoomed && (
          <div
            className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/80 backdrop-blur-sm
