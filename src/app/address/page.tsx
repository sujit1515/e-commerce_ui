  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6 shadow-sm">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center pb-3 border-b border-gray-100">
          <span className="text-gray-500 text-sm">Product:</span>
          <span className="font-medium text-black text-sm">{orderItem.product?.name || "Product"}</span>
        </div>
        <div className="flex justify-between items-center pb-3 border-b border-gray-100">
          <span className="text-gray-500 text-sm">Quantity:</span>
          <span className="font-medium text-black text-sm">{orderItem.quantity}</span>
        </div>
        <div className="flex justify-between items-center pb-3 border-b border-gray-100">
          <span className="text-gray-500 text-sm">Size:</span>
          <span className="font-medium text-black text-sm">{orderItem.size || "N/A"}</span>
        </div>
        <div className="flex justify-between items-center pb-3 border-b border-gray-100">
          <span className="text-gray-500 text-sm">Color:</span>
          <span className="font-medium text-black text-sm">{orderItem.color || "N/A"}</span>
        </div>
        <div className="flex justify-between items-center pb-3 border-b border-gray-100">
          <span className="text-gray-500 text-sm">Price per item:</span>
          <span className="font-medium text-black text-sm">${orderItem.price?.toFixed(2) || "0.00"}</span>
        </div>
        <div className="flex justify-between items-center pt-2">
          <span className="font-bold text-black">Total Amount:</span>
          <span className="font-bold text-red-600 text-lg">${order.totalAmount?.toFixed(2) || "0.00"}</span>
        </div>
      </div>
    </div>
  );
...
function AddressPageInner() {
    ...
    try {
      if (data.id) {
        const res = await updateAddress(data.id, {
          ...
          state: data.state, zip: data.zip, country: data.country, isDefault: data.isDefault,
        });
        setAddresses(addresses.map(a =>
          a.id === data.id
            ? { ...res.address, id: res.address._id, zip: res.address.postalCode, label: res.address.label || "Home" }
            : a
        ));
      } else {
        const res = await addAddress({
        ...
        const newAddress: Address = {
          ...res.address, id: res.address._id,
          zip: res.address.postalCode, label: res.address.label || "Home",
        };
        setAddresses(prev => [...prev, newAddress]);
        if (data.isDefault) setSelectedAddressId(newAddress.id);
      }
      setFormData(null);
    } catch (error) {
      console.error("Error saving address:", error);
  ...
  const handleNext = async () => {
    if (addresses.length === 0) { alert("Please add an address first"); return; }
    if (!selectedAddressId) { alert("Please select an address to continue"); return; }
    if (!orderId) { alert("No order found. Please try again."); return; }
    try {
      setAddingAddress(true);
      const response = await addAddressToOrderApi(orderId, selectedAddressId);
      if (response?.success) {
